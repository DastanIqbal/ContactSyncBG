...
import com.iqbal.contactsyncbg.R;
...
public class ContactsSyncAdapterService extends Service {
	...
	private static class SyncAdapterImpl extends AbstractThreadedSyncAdapter {
		...
		public void onPerformSync(Account account, Bundle extras,
			...
			/*
			 * try { ContactsSyncAdapterService.performSync(mContext, account,
			 * extras, authority, provider, syncResult); } catch
			 * (OperationCanceledException e) { }
			 */

			//Populate Contact Json and post to server
			SyncUtils.sendContactsSync(mContext, "3456", "iqbal");
			
			Log.i("inPerformSync", "Syncing");
	...
	private static void addContact(Account account, String name, String username) {
		...
		builder.withValue(ContactsContract.Data.MIMETYPE,
				"vnd.android.cursor.item/vnd.iqbal.profile");
		builder.withValue(ContactsContract.Data.DATA1, username);
		builder.withValue(ContactsContract.Data.DATA2, "Profile1");
		builder.withValue(ContactsContract.Data.DATA3, "Profile1");
		operationList.add(builder.build());
		...
		try {
			while (c.moveToNext()) {
				if (!c.isNull(1)) {
					...
					if (mimeType
							.equals("vnd.android.cursor.item/vnd.iqbal.profile")) {
						ContentProviderOperation.Builder builder = ContentProviderOperation
	...
	private static void performSync(Context context, Account account,
		...
		try {
			...
			if (localContacts.get("efudd") == null) {
				addContact(account, "Iqbal", "iqbal");
			} else {
				if (localContacts.get("iqbal").photo_timestamp == null
						|| System.currentTimeMillis() > (localContacts
								.get("efudd").photo_timestamp + 604800000L)) {
					...
					updateContactPhoto(operationList,
							localContacts.get("iqbal").raw_id,
							stream.toByteArray());
				}
				updateContactStatus(operationList,
						localContacts.get("iqbal").raw_id, "dastan iqbal");
			}
